...
#ifndef MBED_MATH_COMP_H_
#define MBED_MATH_COMP_H_
...
namespace math_comp {
	...
	template<class T, uint8_t size> class _vector;
	template<class T, uint8_t size> _vector<T, size> operator+ (const _vector<T,size> &x, const _vector<T, size> &y);
	template<class T, uint8_t size> _vector<T, size> operator- (const _vector<T,size> &x, const _vector<T, size> &y);
...
//	template<class T, size_t size> _vector<T, size> operator+ (const _vector<T,size> &x, const _vector<T, size> &y);
	template<class T, uint8_t size> _vector<T, size> operator* (const _vector<T,size> &x, const T &y);
...
//		T operator*(const _vector<T> &, const _vector<T> &);	// Vector by vector multiplication
		...
		T operator[](const uint8_t n) const;										// Get a vector element
//		T const & operator[](size_t n);
		T get_element(const uint8_t n) { return el[n]; }
		void set_element(const T val, const uint8_t n) { el[n] = val; }
//
		void printv();													// Print vector elements
		void setv(const _vector<T, size> &);											// Set vector elements
	};

	template <class T, uint8_t N, uint8_t M>
	class _matrix {
		//unsigned short row;
		//unsigned short col;
...
/*		_matrix<T> operator+(const _matrix<T> &, const _matrix<T> &);
	...
	_vector<T,size>::_vector(const _vector<T,size> &v) {
		...
		for(uint8_t i = 0; i < size; i++) el[i] = v[i];
	...
	template<class T, uint8_t size> T _vector<T,size>::operator[] (const uint8_t n) const {
		return el[n];
	}
/*	template<class T, size_t size> T const & _vector<T,size>::operator[](size_t n) {
	...
	template<class T, uint8_t size> _vector<T, size>& _vector<T,size>::operator= (const _vector<T,size> &x) {
		//_vector<T,size> res;
